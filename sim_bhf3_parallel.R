...
# BIC 
sim_bhf3_parallel <- function(beta, mux, sdx, sdv, sde, rate = 0.1, ni = NULL,maxiter =2000,
                              popn, group,alp = c(0.5,1,1.5),seed = 7231625)
{
  ...
  nrnzero <- 0 
  groupmat1 <- groupmat2 <- rep(0,nobs)
  ...
  {
    res1 <- Spgr_bhf3(indexy = dats$dom,y = dats$y,x = cbind(1,dats$x),weights = c1,betam0 = betam01,lam = lam1[l],maxiter = maxiter)
    betam01 <- res1$beta
  ...
  groupmat1 <- group1[,which.min(bic1)]
  ...
  for(l1 in 1:length(alp))
  {
    ...
    {
      res2l <- Spgr_bhf3(indexy = dats$dom,y = dats$y,x = cbind(1,dats$x),weights  = c2l,betam0 = betam02l,lam = lam2[l2],maxiter = maxiter)
      betam02l <- res2l$beta
  ...
  groupmat2 <- group2[,which.min(bic2)]
  ...
  {
    nrnzero <- nrnzero + 1
    msemat <- cbind((estdir$Direct - meany[,2])^2,
