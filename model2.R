...
rmse_mat2 <- matrix(0, M, 6)
colnames(rmse_mat2) <- c("direct","eblup","reg","ridge","lasso","Alasso")
numcoef2 <- rep(0,M)

for(m in 1: M)
{
  sim1 <- simdat(N = N,Nv = Nv,srh = 0.3,srl = 0.1,Ax = Ax,ax = ax,bx = bx,
                 model = 2,beta = c(100,1),
                 sde = 25,sdv = 25, seed = 2330 + m)
  dats <- sim1$sample
  datp <- sim1$pop
  popn <- sim1$domsize
...
save(rmse_mat2, numcoef2, file = "Research/survey/result/rmse_mat2.RData")
