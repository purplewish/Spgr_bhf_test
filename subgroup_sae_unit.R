...
library(Spgr)
library(Matrix)
source('~/Research/cluster/code/refit.R')
source("~/Research/NRI_urban/code/Spgr_bhf.R")
                        ...
                        alp = c(0.5,1,1.5), seed = 7231625)
{
  ...
  for(m in 1:M)
  {
    ...
    for(l in 1:length(lam1))
    {
      ...
      betam01 <- res1$beta
      beta_array1[,,l] <- res1$beta
      bic1[l] <- BICc_bhf3(obj = res1,c0 = 0.2)
      group1[,l] <- res1$group
    ...
    for(l1 in 1:length(alp))
    {
      ...
      for(l2 in 1:length(lam2))
      {
        ...
        betam02l <- res2l$beta
        beta_array2l[,,l2] <- res2l$beta
        bic2l[l2] <- BICc_bhf3(obj = res2l,c0 = 0.2)
        group2l[,l2] <- res2l$group
...
par(mfrow = c(1,2))
boxplot(sim_bhf2_d05_r025_sde1$mse)
boxplot(sim_bhf2_d05_r01_sde1$mse)

boxplot(sim_bhf2_d1_r01_sde1$mse)
##
